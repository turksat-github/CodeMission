...
class AFTNDatabase:
    ...
    def parse_aftn_message(raw_message: str) -> dict:
        # Clean and split lines
        cleaned = raw_message.strip('\x01\x02\x0b\x03').replace('\r\n', '\n')
        lines = cleaned.split('\n')
        # Find AFTN start (line with priority code)
        priority_codes = ['FF', 'GG', 'SS', 'DD', 'KK', 'LL']
        aftn_start = 0
        for i, line in enumerate(lines):
            if any(line.strip().startswith(code) for code in priority_codes):
                aftn_start = i
                break
        else:
            raise ValueError("No AFTN header found")
        aftn_lines = lines[aftn_start:]
        if len(aftn_lines) < 2:
            raise ValueError("Incomplete AFTN message")
        # Parse priority and addressees
        priority_line = aftn_lines[0].strip().split()
        priority = priority_line[0]
        addressees = priority_line[1:]
        # Parse filing time and originator
        originator_line = aftn_lines[1].strip()
        match = re.match(r'^(\d{6})\s+(\w+)$', originator_line)
        if not match:
            raise ValueError("Invalid originator line")
        filing_time, originator = match.groups()
        # Content
        content = '\n'.join(aftn_lines[2:]).strip()
        return { 'priority': priority, 'addressees': addressees, 'filing_time': filing_time, 'originator': originator, 'content': content }
        ...
        try:
            ...
            while self.is_running:
                ...
                if not data:
                    ...
                    break
                print(f"Received data: {repr(data)}")  # Use repr() to show raw data including newlines
                buffer += data
                print(f"Buffer size: {len(buffer)} bytes")
                if "\n\n" in buffer:
                    messages = buffer.split("\n\n")
                    buffer = messages.pop()  # Keep incomplete message in buffer
                    print(f"Split into {len(messages)} messages; remaining buffer: {repr(buffer)}")
                    for msg in messages:
                        if msg.strip():
                            try:
